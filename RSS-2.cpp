...
With Sorting.
Algoritma dan Struktur Data membuat program Round Robin
...
typedef struct TElmtList{
	...
	infonama infoname; /*Untuk Menyimpan Info Nama elemen P pada sebuah List L yang mempunyai Type Data Char */
...
address Alokasi (infotype X,infonama Y){
...
void InsertVFirst (List *L, infotype X, infonama Y){
...
void Sortir(List *L)
{
    if(Next(First(*L)) == First(*L))    // Is one element
    {
        return; // Return NULL or Stopping The Process
    }
    ...
    do
    {
        ...
        do
        {
            ...
            B = Next(B);    // Next Iteration j++
        }while(B != First(*L)); //batas j<N

        A = Next(A);    // Next Iteration i++
    }while(A != First(*L)); // i<N
}
...
cout<<"\t==================================================================="<<endl;
	...
	cout<<"Jumlah data minimal adalah 3!\n\n"<<endl;
	do{
  		cout<<"Jumlah Data \t\t: ";
  		cin>> N;
  		if (N >= 3){
